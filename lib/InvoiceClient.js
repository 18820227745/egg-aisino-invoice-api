...
class InvoiceClient {
  ...
  async create(params) {
    ...
    data.hjbhsje = params.kplx === '2' ? 0 : _.floor(params.sum, 2); // 合计不含税金额。所有商品行不含税金额之和。
  ...
  async download(params) {
    ...
    console.log(result.returnStateInfo)
    if (result.returnStateInfo.returnCode == '0000') { // PDF_XZFS 1 是pdf内容 必然要解压
      ...
      throw new Error(`\n INVOICE INFO ERROR DOWNLOAD  \t ${result.returnStateInfo.returnCode} \t`)
    }
  }
  ...
  async wsRequest(xml) {
    ...
    const { response: { body } } = await soapRequest({ url: HOST, headers, xml, timeout: 60000 });
