...
class InvoiceClient {
  ...
  async download(params) {
    ...
    if (result.returnStateInfo.returnCode == '0000') { // PDF_XZFS 1 是pdf内容 必然要解压
      ...
      const errMessage = Buffer.from(result.returnStateInfo.returnMessage, 'base64').toString();
      throw new Error(`发票信息获取失败，错误码： ${result.returnStateInfo.returnCode} 错误信息：${errMessage} `)
    }
  }
  ...
  async parseNotificationResult(xmlbody) {
    ...
    invoiceCore.setKey3Des(passWord.slice(-24));
