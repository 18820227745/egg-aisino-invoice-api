...
<interface xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.chinatax.gov.cn/tirip/dataspec/interfaces.xsd" version="DZFP1.0">  
  ...
  <Data> 
    <dataDescription> 
      <zipCode>0</zipCode>  
      <encryptCode>${config.ENCRYPTCODE}</encryptCode>  
      <codeType>0</codeType> 
    </dataDescription>  
    ...
    getContent(params) {
        ...
        config.GHF_NSRSBH = params.ghf_nsrsbh;
        ...
        return this.is3des() ? this.des3Encrypt(root) : util.base64(root);
    };

    is3des() {
        const { config } = this;
        return  config.ENCRYPTCODE === '1';
    }
    ...
    des3Decrypt(encrypted, key = this.config.KEY3DES, needUnGzip = false, needDecrypt = true) {
        const cipher = new Crypto('des-ede3', key);
        let result;
        if (needUnGzip) {
            const bufRes = Buffer.from(encrypted, 'base64');
            const unzipRes = zlib.gunzipSync(bufRes);
            result = needDecrypt ? cipher.decrypt(unzipRes) : unzipRes;
        } else {
    ...
    async decryptByOption(content, { key, zipCode, encryptCode, codeType}) {
        ...
        if (zipCode === '1' && encryptCode === '0' && (codeType === '3DES' || codeType === '0')) {
            result = await this.des3Decrypt(content, key, true, false)
        }
